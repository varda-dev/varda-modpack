...
- Run `.\scripts\reset-sync.ps1` to clean-up the modpack folder and copy over this repo's changes. Use `Get-Help .\scripts\reset-sync.ps1` for help.
...
2. Run `./server-scripts/setup-server.sh` to prepare the server. This creates a user, sets up directories and systemd scripts.
### Restart Cronjob
...
2. Run `.\scripts\prep-server.ps1` to prepare the server files from the Curseforge project.
   * If the Curseforge instance directories aren't in `%userprofile%\curseforge\`, this won't work. Adjust accordingly.
   * This removes unneeded directories and mods, downloads NeoForge, and zips it all up.
3. SCP varda-server.zip over to your home folder.
   * `scp varda-server.zip <IP>:/home/<you>`
4. Run `./server-scripts/setup-minecraft.sh` from the git repo on the server to setup the server files.
